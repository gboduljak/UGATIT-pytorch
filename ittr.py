"""Based on https://github.com/lucidrains/ITTR-pytorch/blob/main/ITTR_pytorch/ITTR_pytorch.py"""
...
class HPB(nn.Module):
  ...
  def __init__(
    ...
    self.ff = nn.ModuleList([
        ...
        Residual(nn.Sequential(
            nn.Conv2d(ff_inner_dim, ff_inner_dim, 3,
                      padding=1, groups=ff_inner_dim),
            norm(ff_inner_dim),
            nn.GELU(),
            nn.Dropout(ff_dropout)
        )),
    ...
    self.ff_pre = nn.ModuleList([
        nn.Conv2d(dim, ff_inner_dim, 1),
        norm(ff_inner_dim),
        nn.GELU(),
        nn.Dropout(ff_dropout),
    ])
    self.ff_main = nn.ModuleList([
        nn.Conv2d(ff_inner_dim, ff_inner_dim, 3,
                  padding=1, groups=ff_inner_dim),
        norm(ff_inner_dim),
        nn.GELU(),
  ...
  def forward(self, x, gamma=None, beta=None):
    if gamma and beta:
      gamma, gamma_ff = gamma
      beta, beta_ff = beta

    attn_branch_out = self.attn(x, gamma, beta)
    ...
    for layer in self.ff_pre:
      if type(layer) == adaILN:
        out = layer(out, gamma_ff, beta_ff)
      else:
        out = layer(out)

    out_skip = out
    for layer in self.ff_main:
      if type(layer) == adaILN:
        out = layer(out, gamma_ff, beta_ff)
      else:
        out = layer(out)
...
class DPSA(nn.Module):
  ...
  def forward(self, x, gamma=None, beta=None):
    ...
    if type(self.norm) == ChanLayerNorm:
      x = self.norm(x)
    else:
    ...
    if need_height_select_and_rank:
      ...
      top_h_indices = einsum('b d, b h d -> b h', q_probe,
                             k_height).topk(k=self.height_top_k, dim=-1).indices

      top_h_indices = repeat(top_h_indices, 'b h -> b h w d',
                             d=self.dim_head, w=k.shape[-2])

      k, v = map(lambda t: t.gather(1, top_h_indices),
                 (k, v))  # first gather across height

    if need_width_select_and_rank:
      k_width = reduce(k, 'b h w d -> b w d', 'sum')

      top_w_indices = einsum('b d, b w d -> b w', q_probe,
                             k_width).topk(k=self.width_top_k, dim=-1).indices

      top_w_indices = repeat(top_w_indices, 'b w -> b h w d',
                             d=self.dim_head, h=k.shape[1])

      k, v = map(lambda t: t.gather(2, top_w_indices),
                 (k, v))  # then gather along width
