...
class ResnetGenerator(nn.Module):
  def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, light=True, nce_layers: List[int] = [0, 5, 9, 12, 16, 20]):
    assert (n_blocks >= 0)
  ...
  def forward(self, input, nce: bool = False):
    # for i, block in enumerate(self.DownBlock):
    #   print(i, str(block))
    x = input
    nce_outs = []

    for (layer_idx, layer) in enumerate(self.DownBlock):
      x = layer(x)
    ...
    if nce:
      return out, cam_logit, heatmap, nce_outs
