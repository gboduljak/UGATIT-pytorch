...
class UpsamplingType(StrEnum):
  PAPER = 'paper'
  CONCAT_SKIP = 'concat-skip'


class ResnetGenerator(nn.Module):
  def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, n_downsampling=2, upsampling_type: UpsamplingType = UpsamplingType.PAPER, light=False):
    assert (n_blocks >= 0)
    ...
    for i in range(n_downsampling):
      ...
      if self.upsampling_type == UpsamplingType.PAPER:
        UpBlock2 += [
            UpsampleBlock(
  ...
  def forward(self, x):
    if self.upsampling_type == UpsamplingType.CONCAT_SKIP:
      down_skips = []
    ...
    if self.upsampling_type == UpsamplingType.PAPER:
      ...
      assert (len(self.UpBlock2) == len(down_skips))
...
class UpsampleWithConcatSkip(nn.Module):
  def __init__(self, in_channels: int, out_channels: int):
    super(UpsampleWithConcatSkip, self).__init__()
  ...
  def forward(self, x, skip):
    x = torch.cat((self.up(x), skip), dim=1)  # concat on channels axis
    x = self.conv(x)  # reduce to out_channels
    return x


class UpsampleBlock(nn.Sequential):
  def __init__(self, in_channels: int, out_channels: int, scale_factor: int, kernel_size: int, stride: int, padding: int, bias: bool, activation: nn.Module):
    super().__init__(
