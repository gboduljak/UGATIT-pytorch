...
from .utils import accuracy, binary_accuracy
...
class DomainAdversarialLoss(nn.Module):
  ...
  def __init__(self, domain_discriminator: nn.Module, reduction: Optional[str] = 'mean',
               grl: Optional[nn.Module] = None, sigmoid=True):
  ...
  def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
    ...
    if self.sigmoid:
      ...
      loss = F.cross_entropy(d, d_label, reduction='none') * \
          torch.cat([w_s, w_t], dim=0)
